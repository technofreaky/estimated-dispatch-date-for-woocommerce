 ...
 * @author keesiemeijer
 */
if ( !class_exists( 'Estimated_Dispatch_Date_For_WooCommerce_WP_Settings_Fields' ) ) {
	class Estimated_Dispatch_Date_For_WooCommerce_Settings_WP_Fields {
		...
		public function callback_multicheckbox( $args ) {
			...
			$fieldClass = '';
			if(isset($args['fieldClass'])){$fieldClass = $args['fieldClass'];}
			$html  = '<fieldset class="'.$fieldClass.'" >';
			$i = 0;
			foreach ( (array) $args['options'] as $opt => $label ) {
